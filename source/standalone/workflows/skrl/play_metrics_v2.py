...
class EvaluationMetrics:
    ...
    def reset_episode_data(self):
        self.episode_data = {
            'phase_reached_count': torch.zeros((self.num_envs, self.num_phases)),
            'phase_reached_timesteps': torch.full((self.num_envs, self.num_phases), -1, dtype=torch.int32),
            ...
            'height_success_mask': torch.zeros(self.num_envs, dtype=torch.bool),
            'obj_drop_mask': torch.zeros(self.num_envs, dtype=torch.bool),
    ...
    def update_height_data(self, obj_positions: torch.Tensor, timestep: int):
        ...
        for env_idx in range(self.num_envs):
            if above_threshold[env_idx]:
                if not self.episode_data['height_success_mask'][env_idx]:
                    self.episode_data['height_success_mask'][env_idx] = True
                    self.episode_data['height_success_timesteps'][env_idx] = timestep
                self.episode_data['obj_drop_mask'][env_idx] = False
            else:
                if self.episode_data['height_success_mask'][env_idx] and timestep < 248:
                    self.episode_data['obj_drop_mask'][env_idx] = True
    ...
    def get_episode_summary(self, episode_idx: int) -> dict:
        ...
        summary = {
            ...
            'phase_success_rates': {
                f'phase_{i+1}': float(torch.sum(data['phase_reached_count'][:, i])) / self.num_envs
                for i in range(self.num_phases)
            },
            'height_success_rate': float(torch.sum(data['height_success_mask'])) / self.num_envs,
            'drop_count_rate': float(torch.sum(data['obj_drop_mask'])) / self.num_envs,
    ...
    def get_overall_summary(self) -> dict:
        ...
        phase_success_rates = {}
        for phase_idx in range(self.num_phases):
            phase_successes = torch.cat([data['phase_reached_count'][:, phase_idx] for data in self.all_episodes_data])
            phase_success_rates[f'phase_{phase_idx+1}'] = {
                'mean': float(torch.mean(phase_successes)),
                'std': float(torch.std(phase_successes)),
                'rate': float(torch.sum(phase_successes)) / (num_episodes * self.num_envs)
            }

        height_successes = torch.cat([data['height_success_mask'].float() for data in self.all_episodes_data])
        height_success_rate = {
            'mean': float(torch.mean(height_successes)),
            'std': float(torch.std(height_successes)),
            'rate': float(torch.sum(height_successes)) / (num_episodes * self.num_envs)
        }

        drop_count = torch.cat([data['obj_drop_mask'].float() for data in self.all_episodes_data])
        drop_count_rate = {
            'rate': float(torch.sum(drop_count)) / (num_episodes * self.num_envs)
        }
...
def main():
    ...
    for episode in range(args_cli.num_episodes):
        ...
        print(f"  - Obj Drop rate: {episode_summary['drop_count_rate']:.2}")
    # Save results
